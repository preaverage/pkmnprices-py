...
class CardListingsResource:
    ...
    def tcgplayer(
        ...
        raw = self._t.request(ep.cards_listings_tcgplayer(
    ...
    def iterate_tcgplayer(self, card_id: int, **params: Any) -> Iterator[TcgplayerListing]:
        return paginate_cursor(lambda cursor: self.tcgplayer(card_id, **{**params, "cursor": cursor}))

    def all_tcgplayer(self, card_id: int, **params: Any) -> List[TcgplayerListing]:
        return list(self.iterate_tcgplayer(card_id, **params))
...
class SealedResource:
    ...
    def get(self, sealed_id: int, *, currency: str | None = None) -> Sealed:
        return Sealed.from_dict(self._t.request(ep.sealed_get(sealed_id, currency=currency)))
